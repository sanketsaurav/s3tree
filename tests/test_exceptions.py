...
from s3tree.exceptions import (
    ImproperlyConfiguredError, BucketNotFound, BucketAccessDenied,
    InvalidPathError, FileNotFound, DirectoryNotFound)
...
def test_improperly_configured_error_exc():
    ...
    assert str(exc.value) == """aws_access_key_id and aws_secret_access_key must be
...
def test_bucket_access_denied_exc():
    ...
    assert str(exc.value) == ("Permission to access denied for the"
                              " bucket: dummy")
...
def test_invalid_path_error_exc():
    ...
    assert str(exc.value) == ("Invalid path provided."
